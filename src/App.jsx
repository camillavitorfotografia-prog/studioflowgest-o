...
import './styles/mobileAccessibility.css';
import {
  applyStoredTheme,
  applyTheme,
  isSettingsStorageEvent,
  THEME_EVENT,
} from './utils/theme';
...
const ModelosContratos = lazy(() => import('./pages/Configuracoes/ModelosContratos'));
const ModelosPropostas = lazy(() => import('./pages/Configuracoes/ModelosPropostas'));
const ContractTemplateEditor = lazy(() => import('./features/documents/editor/ContractTemplateEditor'));
const ProposalTemplateEditor = lazy(() => import('./features/documents/editor/ProposalTemplateEditor'));
const AreaCliente = lazy(() => import('./pages/AreaCliente'));
const PortalCliente = lazy(() => import('./pages/PortalCliente'));
const BibliotecaArquivos = lazy(() => import('./pages/BibliotecaArquivos'));
const Galerias = lazy(() => import('./pages/Galerias'));
const GaleriaPublica = lazy(() => import('./pages/GaleriaPublica'));
const GaleriaPreview = lazy(() => import('./pages/GaleriaPreview'));
...
function ThemeSync() {
  useEffect(() => {
    ...
    const onTheme = (event) => applyTheme(event?.detail?.theme);
    const onStorage = (event) => {
      if (isSettingsStorageEvent(event)) sync();
    };

    window.addEventListener(THEME_EVENT, onTheme);
    window.addEventListener('storage', onStorage);
    window.addEventListener('studioflow:account-scope-changed', sync);

    return () => {
      window.removeEventListener(THEME_EVENT, onTheme);
      window.removeEventListener('storage', onStorage);
      window.removeEventListener('studioflow:account-scope-changed', sync);
    };
  }, []);
