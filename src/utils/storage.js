...
const normalizeRecord = (record, defaults) => ({
  ...defaults,
  ...asObject(record),
});
...
const TRANSACTION_DEFAULTS = {
  descricao: 'Lançamento financeiro',
  categoria: 'Outros',
  valor: 0,
  vencimento: '',
  dataRecebimento: '',
...
export const normalizeStoredValue = (key, value) => {
  ...
  if (key === STORAGE_KEYS.leads) {
    return asArray(value).map((item) => normalizeRecord(item, LEAD_DEFAULTS));
  }

  if (key === STORAGE_KEYS.clients) {
    return asArray(value).map((item) => normalizeRecord(item, CLIENT_DEFAULTS));
  }

  if (key === STORAGE_KEYS.projects) {
    return asArray(value).map((item) => {
      const project = normalizeRecord(item, PROJECT_DEFAULTS);
  ...
  if (key === STORAGE_KEYS.contracts) {
    return asArray(value).map((item, index) => (
      normalizeContract(
        normalizeRecord(item, CONTRACT_DEFAULTS),
        index,
      )
    ));
  }
  ...
  if (key === STORAGE_KEYS.equipment) {
    return asArray(value).map((item) => normalizeRecord(item, EQUIPMENT_DEFAULTS));
  }
  ...
  if (key === STORAGE_KEYS.finances) {
    return asArray(value).map((item, index) => {
      ...
      const tipoGeral = norm.tipoGeral
        || (['fixa', 'variavel'].includes(norm.tipo) ? 'Saida' : 'Entrada');
      const tipo = norm.tipo
        || (tipoGeral === 'Saida' ? 'fixa' : 'receita_avulsa');
      const status = norm.status
        || (tipoGeral === 'Saida' ? 'Pendente' : 'prevista');
      const vencimento = norm.vencimento || norm.data || '';
      const competencia = norm.competencia
        || (
          vencimento
  ...
  if (key === STORAGE_KEYS.recurrences) {
    return asArray(value).map((item, index) => {
      ...
      return {
        ...norm,
        id,
        diaVencimento: Math.max(
          1,
          Math.min(31, Number(norm.diaVencimento || 1)),
        ),
