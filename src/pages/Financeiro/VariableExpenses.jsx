...
import { getIncomeNatureLabel, isNonOperationalIncome, isPersonalExternalIncome } from '../../utils/incomeClassification';
...
export default function VariableExpenses(){
  ...
  const fundingEntries=useMemo(()=>{
    ...
    const expenses=finances.filter((item)=>String(item.tipoGeral||item.tipo_geral||'').toLowerCase()==='saida');
    const currentFinanceIds=new Set([form.id,form.grupoParcelamentoId].filter(Boolean).map(String));
    const usedByEntry=new Map();
    expenses.forEach((expense)=>{
      if(currentFinanceIds.has(String(expense.id)) || currentFinanceIds.has(String(expense.grupoParcelamentoId||''))) return;
      const details=expense.detalhes&&typeof expense.detalhes==='object'?expense.detalhes:{};
      const directId=expense.entradaOrigemId||details.entradaOrigemId;
      const total=Number(expense.valorTotal??details.valorTotal??expense.valor??0);
      if(directId) usedByEntry.set(String(directId),(usedByEntry.get(String(directId))||0)+total);
      const composition=Array.isArray(expense.composicaoRecursos)?expense.composicaoRecursos:(Array.isArray(details.composicaoRecursos)?details.composicaoRecursos:[]);
      composition.forEach((line)=>{ if(line?.entradaId) usedByEntry.set(String(line.entradaId),(usedByEntry.get(String(line.entradaId))||0)+Number(line.valor||0)); });
    });
    return finances
      .filter((item)=>String(item.tipoGeral||item.tipo_geral||'').toLowerCase()==='entrada')
      .filter((item)=>isPersonalExternalIncome(item)||isNonOperationalIncome(item))
      .filter((item)=>{
        const status=String(item.status||'').toLowerCase();
        return status!=='cancelada' && (['recebida','recebido','pago','confirmado'].includes(status)||Boolean(item.dataRecebimento||item.data_pagamento||item.dataPagamento));
      })
      .map((item)=>{
        const used=usedByEntry.get(String(item.id))||0;
        const amount=Number(item.valor||0);
        return {...item,valorUtilizado:used,saldoDisponivel:Math.max(0,amount-used),naturezaLabel:getIncomeNatureLabel(item)};
      })
  ...
  },[items,open,form.id,form.grupoParcelamentoId]);
  const eligibleFundingEntries=useMemo(()=>{
    const wanted=NON_OPERATIONAL_CATEGORY_BY_ORIGIN[form.origemRecursosTipo];
    if(form.origemRecursosTipo==='aporte_pessoal') return fundingEntries.filter((item)=>isPersonalExternalIncome(item)||item.categoria===wanted);
  ...
  const save=async()=>{
    ...
    if(form.categoria==='Equipamentos' && selectedFunding && form.origemRecursosTipo!=='origem_mista' && Number(selectedFunding.saldoDisponivel||0)+0.01<total){
      alert(`A entrada selecionada possui apenas ${formatCurrency(selectedFunding.saldoDisponivel||0)} disponível. Informe outra origem ou use origem mista.`);return;
    }
    ...
    const baseId=form.id||makeId(); const rows=Array.from({length:count},(_,i)=>({
      ...form,origemRecursos:resourceDescription,composicaoRecursos:compositionPayload,
      fonteRecursosNatureza:selectedFunding?getIncomeNatureLabel(selectedFunding):resourceLabel,
      fonteRecursosDescricao:selectedFunding?.descricao||selectedFunding?.categoria||'',
      fonteRecursosValor:Number(selectedFunding?.valor||0),valorFonteUtilizado:selectedFunding?total:0,
      id:count===1?baseId:`${group}-${i+1}`,tipo:'variavel',tipoGeral:'Saida',valor:Math.round((total/count)*100)/100,valorTotal:total,
