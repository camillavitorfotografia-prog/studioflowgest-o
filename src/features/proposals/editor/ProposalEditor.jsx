...
import { enrichPricingOptions, getPackageOptionForPage } from '../services/packageSuggestions';
import PublishedProposalEditor from './PublishedProposalEditor';
import '../styles/proposals.css';

export default function ProposalEditor() {
  const { state: incoming = {} } = useLocation();
  return incoming.publishedTemplateId ? <PublishedProposalEditor incoming={incoming} /> : <LegacyProposalEditor incoming={incoming} />;
}

function LegacyProposalEditor({ incoming }) {
  ...
  const generate = async () => {
    ...
    const result = await generateProposalPdf({ template, proposal: build() });
    setStatus('gerada');
    saveProposal({ ...build(), status: 'gerada', pdfFileName: result.fileName });
  ...
  return (
    <section className="proposal-editor">
      <header className="proposal-topbar">
        <div>
          <span>Editor de propostas</span>
          <h1>{template.name}</h1>
        </div>
        <select value={modelId} onChange={(event) => { setModelId(event.target.value); setPageIndex(0); setAssets({}); }}>
          {PROPOSAL_TEMPLATES.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
        </select>
        <select value={clientId} onChange={(event) => setClientId(event.target.value)}>
          <option value="">Selecionar cliente</option>
          {clients.map((item) => <option key={item.id} value={item.id}>{item.nome || item.name}</option>)}
        </select>
        <button onClick={() => setPricing(capturePricingSnapshot())}>Atualizar precificação</button>
        <span className="proposal-status">{status}</span>
        <button onClick={save}><Save />Salvar</button>
        <button onClick={generate}><Download />Gerar PDF</button>
      </header>
      ...
      <div className="proposal-workspace">
        ...
        <main className="proposal-stage">
          ...
          <ProposalPageCanvas
            ...
            pricing={pricing}
            pricingOptions={normalizedIncomingOptions}
            packageOption={pagePackageOption}
            logo={logo}
            zoom={zoom}
            onSelectImage={() => {}}
          />
        ...
        <aside className="proposal-inspector">
          ...
          <button className="proposal-preview"><Eye />Pré-visualizar página</button>
        </aside>
      </div>
    </section>
  );
