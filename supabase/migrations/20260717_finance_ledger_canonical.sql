-- Fonte canônica e não destrutiva do Financeiro.
-- A view classifica cada lançamento sem alterar ou apagar registros históricos.
create or replace view public.finance_ledger_canonical
...
select
  ...
  coalesce(
    ...
    case when coalesce(f.data_vencimento, '') ~ '^\d{4}-\d{2}-\d{2}' then substring(f.data_vencimento from 1 for 10)::date end
  ) as effective_date,
  f.created_at,
  f.updated_at
from public.financas f;

create index if not exists financas_user_type_date_idx
  on public.financas (user_id, tipo, data_pagamento, data);
create index if not exists financas_user_general_status_idx
  on public.financas (user_id, tipo_geral, status);
