-- Separa receitas pessoais externas das receitas operacionais da empresa.
-- Os metadados fiscais permanecem em financas.detalhes para manter compatibilidade
-- com o schema atual da tabela financas.
...
select
  ...
  case
    ...
    when lower(coalesce(f.tipo_geral, '')) = 'entrada'
      and lower(coalesce(f.detalhes->>'naturezaFinanceira', '')) = 'pessoal_externa'
      then 'personal_external_income'
    when lower(coalesce(f.tipo_geral, '')) = 'entrada' and (
      ...
      or lower(coalesce(f.categoria, '')) in (
        'aporte pessoal da titular','aporte do titular','venda de patrimônio da empresa',
        'venda de patrimonio da empresa','venda de patrimônio','venda de patrimonio',
